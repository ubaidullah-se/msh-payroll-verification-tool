  ...
  Array.from(timesheetEmps)
    ...
    .forEach((tsName) => {
      ...
      const payrollOptions =
        ...
        Object.keys(payrollEmps)
          .map((p) => `<option value="${p}">${capitalizeName(p)}</option>`)
          .join("");
      // Dept options
      const deptOptions = Array.from(payrollDepts)
      ...
      const exact = Object.keys(payrollEmps).find(
        (p) => norm(p) === norm(tsName),
      );
      if (exact) {
        document.getElementById(
          `map-${tsName.replace(/[^a-zA-Z0-9]/g, "_")}`,
        ).value = exact;
        document.getElementById(
          `dept-${tsName.replace(/[^a-zA-Z0-9]/g, "_")}`,
        ).value = payrollEmps[exact].dept;
      }
    });
}
...
function parseTimesheet(raw, label) {
  ...
  for (let r = 0; r < raw.length; r++) {
    ...
    const isTimingRow = ["Time In", "Time Out", "BREAK", "Hours"].some((k) =>
      c0.includes(k),
    );
    const isHeaderRow =
...
async function runAudit() {
  ...
  try {
    ...
    const allKeys = new Set([...Object.keys(w1.emps), ...Object.keys(w2.emps)]);
    const biweekly = {};
    allKeys.forEach((k) => {
